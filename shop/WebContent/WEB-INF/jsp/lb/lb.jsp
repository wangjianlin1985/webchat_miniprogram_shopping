...
<div class="page-container">
  ...
  <%--  <div class="text-c"> 
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
	    <span class="l">  
	                      <a href="javascript:;" onclick="admin_add('添加类别','${ctx}/lb_add.do','600','300')" class="btn btn-primary radius">
	                      <i class="Hui-iconfont">&#xe600;</i>添加类别</a></span>
	                     <span class="r">
	                     <div class="panel-foot text-center">
      <pg:pager  url="${ctx}/lb_lb.do" maxIndexPages="5" items="${pagers.total}"  maxPageItems="15" export="curPage=pageNumber" >
		<pg:last>  
	...
	private String ms;//描述 -->
		...
		<tbody>
		 ...
		 <tr class="text-c">
				<td>${man.name}</td>
				<td class="td-manage">
				 <a title="品牌管理" href="${ctx }/pp_pp.do?lbId=${man.id}" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">品牌管理</i></a>
				 <a title="编辑" href="${ctx }/lb_update.do?id=${man.id}" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
function admin_add(title,url,w,h){
	window.location.href=ctx+"/lb_add.do"
}
/*管理员-删除*/
function admin_del(id){
	layer.confirm('确认要删除吗？',function(index){
		...
		window.location.href=ctx+"/lb_delete.do?id="+id;
	});
}
/*管理员-编辑*/
